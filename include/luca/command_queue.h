...
struct luca_command_queue { pthread_mutex_t mu; struct luca_command items[LUCA_CMD_QUEUE_CAP]; unsigned head, tail, count; int wake_read,wake_write; };
...
int luca_command_wake_fd(struct luca_command_queue *q);
void luca_command_drain_wake(struct luca_command_queue *q);
